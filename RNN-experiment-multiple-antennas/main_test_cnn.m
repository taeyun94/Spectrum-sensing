...
load('./network/CNN_net_AWGN_timeblock_64.mat');

load('Experiment_NLOS_multiAnt_CNN_data.mat');
...
for loop=1:length(YData(1,:))
    temp = classify(net,XData(:,:,1,:,loop));
...
SS = sprintf('./performance/CNN_NLOS_performance_multiple_ant');
save(SS, 'MDR', 'FDR', 'ACC', 'estimated_SNR');
