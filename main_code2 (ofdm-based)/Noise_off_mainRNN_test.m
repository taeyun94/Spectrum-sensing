function Noise_off_mainRNN_test(time_block)

S = sprintf('./network/Noise_off_RNN_net_timeblock_%d', time_block);
load(S);

SS = sprintf('Noise_off_test_data_OFDM_rnn_timeblock_%d.mat', time_block);
load(SS);
...
%     temp = classify(net,XTest_rnn(:,:,:,:,loop));
    temp = classify(net,XTest_rnn(:,loop));
...
SSS = sprintf('./performance/Noise_off_RNN_timeblock_%d_2', time_block);
