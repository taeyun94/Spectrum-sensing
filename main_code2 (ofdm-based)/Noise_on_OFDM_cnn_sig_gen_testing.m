function Noise_on_OFDM_cnn_sig_gen_testing(time)
% clc
% clear
...
XTest_cnn = zeros(2*width, N_time_block, 1, N_test*N_ch,length(SNR_table));
...
%     SNR_table(SNR_loop)
    ...
    for loop=1:N_test
        ...
        for n=1:N_ch
            XTest_cnn(:,:,1,(loop-1)*N_ch+n,SNR_loop) = [rx_sig_mat(width*(n-1)+1:width*n,:); rx_sig_mat(end-width+1:end,:)];
        end
...
S = sprintf('Noise_on_test_data_OFDM_cnn_timeblock_%d.mat', N_time_block);
