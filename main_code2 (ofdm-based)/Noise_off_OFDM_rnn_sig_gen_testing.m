function Noise_off_OFDM_rnn_sig_gen_testing(time)
% clc
% clear
...
XTest = zeros(width, N_time_block, 1, N_test*N_ch,length(SNR_table));
...
%     SNR_table(SNR_loop)
    ...
    for loop=1:N_test
        ...
        for n=1:N_ch
            XTest(:,:,1,(loop-1)*N_ch+n,SNR_loop) = rx_sig_mat(width*(n-1)+1:width*n,:);
        end
        
        for n=1:length(XTest(1,1,1,:,1))
            XTest_rnn{n,SNR_loop} = XTest(:,:,1,n,SNR_loop);
        end
...
S = sprintf('Noise_off_test_data_OFDM_rnn_timeblock_%d.mat', N_time_block);
