    ...
    for loop=1:N_training
        ...
        for n=1:length(Fc_table)
            if ch_status(n) == 1
                ...
                %%% 191015
                SNR = rand(1)*(max_SNR - min_SNR)+min_SNR;
                temp = sqrt(10^(SNR/10))*temp;
                %%%
                if exist('tx_sig','var')
...
%         imshow(rx_sig_mat);
%         figure(2); hold off;
%         imshow(rx_sig_mat(end-width+1:end,:));
        %%%
    ...
    save training_data_set_AWGN XTrain YTrain im_h im_w -v7.3
...
if ~exist('test_data_set_AWGN.mat','file')
    ...
    save test_data_set_AWGN XTest YTest -v7.3
